"""Sample9 Handler - 비동기 DB 동시 쿼리 패턴
...
pg_queries = aiosql.from_path(Path(__file__).parent.parent / "group1" / "sql" / "postgres.sql", "asyncpg")
mysql_queries = aiosql.from_path(Path(__file__).parent.parent / "group2" / "sql" / "mysql.sql", "asyncmy")
...
class Sample9Handler(BaseHandler):
    ...
    async def execute(self, params: HandlerParams) -> HandlerResult:
        ...
        logger.info(
            f"Sample9Handler: fetched stats from 3 DBs "
