"""Sample2 Handler - PostgreSQL 단일 DB CRUD"""
        ...
        if params.action == 'write':
            ...
            async def write_data():
                ...
                row = await queries.insert_sample_data_returning(
                    ...
                    writer_handler='sample2'
                )
                return row['id']

            data_id = await write_data()
            logger.info(f"Sample2Handler: wrote data id={data_id}")
            ...
            logger.info(f"Sample2Handler: read {len(rows)} rows")
