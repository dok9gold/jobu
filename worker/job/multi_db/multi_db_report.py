...
읽기 전용 멀티 DB 예제: 3개 데이터베이스에서 데이터 집계.
@transactional_readonly로 여러 DB를 동시에 읽기 전용으로 접근.
"""
...
sqlite_queries = aiosql.from_path(Path(__file__).parent / "sql" / "multi_db_report_sqlite.sql", "aiosqlite")
pg_queries = aiosql.from_path(Path(__file__).parent / "sql" / "multi_db_report_postgres.sql", "asyncpg")
mysql_queries = aiosql.from_path(Path(__file__).parent / "sql" / "multi_db_report_mysql.sql", "asyncmy")


@handler("multi_db_report")
class MultiDbReportHandler(BaseHandler):
    """3개 DB에서 데이터 집계 리포트 예제 (읽기 전용)"""
    ...
    async def execute(self, params: HandlerParams) -> HandlerResult:
        ...
        logger.info(f"MultiDbReportHandler: report generated, total={report['counts']['total']}")
